...
void setup() {
  ...
  while (!Serial) { // Loop as long as Serial is not yet connected
    if (millis() - serial_connect_start_time > 5000) { // Check if 5 seconds have passed
      break; 
    }
    delay(100); 
  }
  ...
  pinMode(greenLedPin, OUTPUT); digitalWrite(greenLedPin, LOW);
  pinMode(blueLedPin, OUTPUT); digitalWrite(blueLedPin, LOW);
  pinMode(statusLedPin, OUTPUT); digitalWrite(statusLedPin, LOW); // Status LED off
  ...
  Serial.print("Input tensor dimensions: "); Serial.println(model_input->dims->size);
  bool input_params_ok = true;
  if (model_input->dims->size != 4) { Serial.println("ERROR: Input dims->size != 4"); input_params_ok = false; }
  if (model_input->dims->data[0] != 1) { Serial.println("ERROR: Input batch_size != 1"); input_params_ok = false; }
  if (model_input->dims->data[1] != g_num_mfcc_features) { Serial.print("ERROR: Input height != "); Serial.println(g_num_mfcc_features); input_params_ok = false; }
  if (model_input->dims->data[2] != g_num_mfcc_frames) { Serial.print("ERROR: Input width != "); Serial.println(g_num_mfcc_frames); input_params_ok = false; }
  if (model_input->dims->data[3] != g_num_channels) { Serial.print("ERROR: Input channels != "); Serial.println(g_num_channels); input_params_ok = false; }
  if (model_input->type != kTfLiteInt8) { Serial.println("ERROR: Input type != kTfLiteInt8"); input_params_ok = false; }

  // Check quantization parameters of the model's input tensor
  if (abs(model_input->params.scale - g_input_scale) > 1e-6) { // Compare floats with tolerance
      Serial.print("ERROR: Input scale mismatch! Model expects: "); Serial.print(model_input->params.scale, 7);
      Serial.print(" You defined: "); Serial.print(g_input_scale, 7); input_params_ok = false;
  }
  if (model_input->params.zero_point != g_input_zero_point) {
      Serial.print("ERROR: Input zero_point mismatch! Model expects: "); Serial.print(model_input->params.zero_point);
      Serial.print(" You defined: "); Serial.print(g_input_zero_point); input_params_ok = false;
  }
  ...
  Serial.print("Expected "); Serial.print(g_input_tensor_size); Serial.println(" bytes per inference.");
}
...
void loop() {
  ...
  if (Serial.available() > 0) {
    if (Serial.read() == 'S') { 
      ...
      while(Serial.available()) Serial.read(); // Clear any trailing bytes after 'S'
      ...
      if (bytes_read == g_input_tensor_size) {
        ...
        if (predicted_category_index != -1) {
          ...
          if (strcmp(command, "up") == 0) { // Assuming "up" maps to RED
            digitalWrite(redLedPin, HIGH);
            Serial.println("Action: RED LED ON");
          } else if (strcmp(command, "off") == 0) { // All LEDs are off
            Serial.println("Action: 'OFF' command - ALL LEDs OFF");
          } else if (strcmp(command, "on") == 0) {
            digitalWrite(greenLedPin, HIGH); // Simple example: turn green on for "on"
            Serial.println("Action: 'ON' command - GREEN LED ON");
          } else if (strcmp(command, "_silence_") == 0) {
            digitalWrite(blueLedPin, HIGH); // Simple example: turn blue on for "silence"
            Serial.println("Action: 'SILENCE' command - BLUE LED ON");
          }
          else { // Unknown Command Handling or other
        ...
        } else { // Should not happen if predicted_category_index is initialized to -1 and max_score_val is high
      ...
      } else { // Bytes_read was NOT equal to g_input_tensor_size
